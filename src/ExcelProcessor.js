...
const ExcelProcessor = () => {
    ...
    // Crea e scarica un file template di esempio
const downloadTemplate2 = () => {
    try {
        // Crea un nuovo workbook
        const wb = XLSX.utils.book_new();
        
        // Crea il foglio Database con le colonne di esempio
        const databaseData = [
            ['Gender', 'Line', 'Merchandising Class', 'Size Code', 'ORDER QTY', 'SOLD QTY'],
            ['Woman', 'RTW', 'Dresses', 'XS', 10, 8],
            ['Woman', 'RTW', 'Dresses', 'S', 15, 12],
            ['Woman', 'RTW', 'Dresses', 'M', 20, 16],
            ['Woman', 'RTW', 'Dresses', 'L', 15, 10],
            ['Woman', 'RTW', 'Dresses', 'XL', 10, 6],
            ['Woman', 'RTW', 'Shirts', 'XS', 8, 5],
            ['Woman', 'RTW', 'Shirts', 'S', 12, 9],
            ['Woman', 'RTW', 'Shirts', 'M', 16, 12],
            ['Woman', 'RTW', 'Shirts', 'L', 12, 8],
            ['Woman', 'RTW', 'Shirts', 'XL', 8, 4],
            ['Man', 'RTW', 'Suits', '48', 5, 3],
            ['Man', 'RTW', 'Suits', '50', 8, 6],
            ['Man', 'RTW', 'Suits', '52', 10, 8],
            ['Man', 'RTW', 'Suits', '54', 8, 5],
            ['Man', 'RTW', 'Shirts', 'S', 10, 8],
            ['Man', 'RTW', 'Shirts', 'M', 15, 12],
            ['Man', 'RTW', 'Shirts', 'L', 12, 9],
        ];
        
        // Crea il foglio Database
        const databaseWs = XLSX.utils.aoa_to_sheet(databaseData);
        XLSX.utils.book_append_sheet(wb, databaseWs, 'Database');
        
        // Crea un foglio di esempio per mostrare il risultato
        const exampleData = [
            ['Merchandising Class: Dresses'],
            [],
            ['Size Code', 'ORDER QTY', 'SOLD QTY', 'SELL-OUT % by Size', 'S/T %', 'Proposta Split'],
            ['XS', 10, 8, '15,38%', '80,00%', 15],
            ['S', 15, 12, '23,08%', '80,00%', 23],
            ['M', 20, 16, '30,77%', '80,00%', 31],
            ['L', 15, 10, '19,23%', '66,67%', 19],
            ['XL', 10, 6, '11,54%', '60,00%', 12],
            ['TOTALE', 70, 52, '100%', '74,29%', 100],
            [],
            [],
            ['Merchandising Class: Shirts'],
            [],
            ['Size Code', 'ORDER QTY', 'SOLD QTY', 'SELL-OUT % by Size', 'S/T %', 'Proposta Split'],
            ['XS', 8, 5, '13,16%', '62,50%', 13],
            ['S', 12, 9, '23,68%', '75,00%', 24],
            ['M', 16, 12, '31,58%', '75,00%', 32],
            ['L', 12, 8, '21,05%', '66,67%', 21],
            ['XL', 8, 4, '10,53%', '50,00%', 10],
            ['TOTALE', 56, 38, '100%', '67,86%', 100],
        ];
        
        // Crea il foglio di esempio
        const exampleWs = XLSX.utils.aoa_to_sheet(exampleData);
        XLSX.utils.book_append_sheet(wb, exampleWs, 'Woman - RTW');
        
        // Scarica il file
        XLSX.writeFile(wb, 'Esempio.xlsx');
        
    } catch (err) {
        setError(`Errore durante la creazione del template: ${err.message}`);
    }
};
